...
class DeviceCard(QFrame):
    def __init__(self, name, icon, color):
        ...
        name_lbl.setStyleSheet(f"color:#c5cdd9;background:transparent;")
        ...
        self._info_lbl.setStyleSheet("color:#4a5568;background:transparent;")
        layout.addWidget(self._info_lbl)

        # Status label
        self._status_lbl = QLabel("Disconnected")
        self._status_lbl.setFont(QFont("Segoe UI",10,600))
        self._status_lbl.setStyleSheet("color:#4a5568;background:transparent;")
    ...
    def set_connected(self, ip="", port="", extra=""):
        ...
        self.setStyleSheet(
            f"QFrame{{background:#0d0f14;border:1px solid {self._color}44;"
            f"border-radius:8px;}}")
        info = f"{ip}:{port}" if ip else extra
        self._info_lbl.setText(info)
        self._info_lbl.setStyleSheet("color:#8892a4;background:transparent;")
    ...
    def set_disconnected(self, info=""):
        self._set_dot("disconnected")
        self.setStyleSheet(
            "QFrame{background:#0d0f14;border:1px solid #1e2433;border-radius:8px;}")
...
class SummaryPanel(QWidget):
    ...
    def _build_header(self, layout):
        ...
        title.setStyleSheet("color:#c5cdd9;background:transparent;")
        self._ts_lbl = lbl("—","#4a5568",11)
        ...
        self._conn_lbl = lbl("devices connected","#4a5568",10)
    ...
    def _build_info(self, layout):
        ...
        for label, attr in [
            ...
            col.addWidget(lbl(label,"#4a5568",9,True))
            val = lbl("checking...","#8892a4",11)
    ...
    def _refresh(self):
        ...
        color = "#22c55e" if connected == total else "#4a9eff" if connected > 0 else "#4a5568"
